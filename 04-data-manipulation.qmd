...
[later](04-data-manipulation.html#subsetting), but first we'll discuss how to
modify the [basic command
syntax](01-the-basics-of-stata.html#basic-command-syntax) to run a command only
...
data](02-working-with-data-sets.html#loading-subsets-of-the-data).
...
labels](03-data-management.html#labeling-values) to associate 0 and 1 with
...
fact.], [conditional
statements](04-data-manipulation.html#restricting-commands-to-subsets) return 1
...
[`if`](04-data-manipulation.html#restricting-commands-to-subsets). E.g., we see
the following error:
...
In Stata, under the [One Data](01-the-basics-of-stata.html#one-data) principal,
any information in the data^[We'll see some exceptions to this in the
[programming](05-programming.html) section.] must be in a variable. This
...
[drop](04-data-manipulation.html#discarding-data) some data) then `_N` may be
updated.
...
discuss [sorting](04-data-manipulation.html#sorting) later, it may be useful to
...
command](01-the-basics-of-stata.html#short-commands) for "egenerate"; the full
...
[Earlier](04-data-manipulation.html#generating-new-variables) we created the
...
can set the [type](03-data-management.html#describing-the-data) manually
(instead of letting Stata choose the best type
[automatically](03-data-management.html#compressing-data)), and `generate` can
place the new variable as desired rather than [using
`order`](03-data-management.html#changing-variable-ordering). Clearly, neither
...
editing](02-working-with-data-sets.html#editing-data-manually) the data like
this produces a `replace` call that we can move into our [Do file for
reproducibility](01-the-basics-of-stata.html#do-files). However, this `replace`
...
use a [conditional
statement](04-data-manipulation.html#restricting-commands-to-subsets) to
...
- several numbers, either
    - a [numlist](02-working-with-data-sets.html#loading-subsets-of-the-data) as
...
before](04-data-manipulation.html#restricting-commands-to-subsets), by using the
...
[`preserve` and
`restore`](02-working-with-data-sets.html#temporarily-preserving-and-restoring-data)
...
[later](04-data-manipulation.html#sorting)) first and running the `by` statement
...
variable](04-data-manipulation.html#system-variables) first.
...
not to group by. For example, let's say your data consisted of doctors visits
for patients, where patients may have more than one appointment. You want to
generate a variable indicating whether a visit is the first by the patient. Say
`id` stores the patient id, `date` stores the date of the visit.

```stata
bysort id (date): gen firstvisit = _n == 1
```

By placing `date` in parentheses, this ensures that within each `id`, the data
is sorted by `date`. Therefore the first row for each patient is their first
visit, so `_n == 1` evaluates to 1 only in that first row and 0 zero otherwise.
...
`bysort`](04-data-manipulation.html#repeat-commands-on-subsets). We can also
sort as a standalone operation. As before, consider generating a [original
ordering variable](04-data-manipulation.html#system-variables) first.
...
values](04-data-manipulation.html#conditional-variable-generation). When sorting
...
labels](03-data-management.html#labeling-values) as we've previously discussed.
...
If we had called [`compress`](03-data-management.html#compressing-data) on the
...
numerical with [value labels](03-data-management.html#labeling-values) attached.
...
forward (except `census9` from [Exercise 3](03-data-management.html#exercise-3)
...
2. The average age of all Americans in 1980 is roughly 30.11 years of age.
   [Generate a
   categorical](04-data-manipulation.html#conditional-variable-generation) with
   four values as described before, with appropriate [value
   labels](03-data-management.html#labeling-values).
...
3. What is the death rate in [each of those four
   categories](04-data-manipulation.html#repeat-commands-on-subsets)? (You can
   use `summarize` to obtain the means.) Does there appear to be any pattern?
4. What state has the [lowest](04-data-manipulation.html#sorting) death rate?
...
5. Each state has a single observation here, but if we had multiple years of
   ...
   data"](04-data-manipulation.html#reshaping-files) with multiple rows per
   state. To prepare for this sort of data,
   [encode](04-data-manipulation.html#converting-strings-into-labeled-numbers)
...
website](02-working-with-data-sets.html#stata-website-data) which we can append,
"odd" and "even".
...
[match-merging](04-data-manipulation.html#match-merging-data)), it will only
...
directory](01-the-basics-of-stata.html#working-directory) rules; if the file
...
directory](01-the-basics-of-stata.html#working-directory) rules.)
...
We'll use
[`decode`](04-data-manipulation.html#converting-strings-into-labeled-numbers) to
...
- If you have wide data and many time-varying variables, there is no shorthand
  ...
  [let me know](index.html#contact-information)!
