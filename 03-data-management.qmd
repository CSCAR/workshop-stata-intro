...
syntax](01-the-basics-of-stata.html#basic-command-syntax), we said that the
...
[`order`](03-data-management.html#changing-variable-ordering) command later to
re-order variables.
...
[short names](01-the-basics-of-stata.html#short-commands)). We'll see in a
...
[`compress`](03-data-management.html#compressing-data), but for now they all
...
see [`compress`](03-data-management.html#compressing-data) for details.)
...
[label](03-data-management.html#labels) section for further details.
...
As mentioned [above](03-data-management.html#describing the data), there are
...
in the [properties pane](01-the-basics-of-stata.html#the-stata-environment).)
...
1. "census9" is accesible via
   [`webuse`](02-working-with-data-sets.html#stata-website-data). Load it.
2. Spend a minute looking at the data. What does this data seem to represent?
   What variables do we have?
   ([`describe`](03-data-management.html#describing-the-data) will come in handy
   here!)
3. Are there any missing states?
4. What variables (if any) are numeric and what variables (if any) are strings?
5. [Compress](03-data-management.html#compressing-data) the data. How much space
...
discussed [earlier](02-working-with-data-sets.html#colors-as-variable-type).
...
variables](03-data-management.html#referring-to-variables). Recall that
...
1. Going forward, we'll be using a version of "census9" with changes we're
   making. [Save](02-working-with-data-sets.html#saving-data) a copy of the data
...
2. The `drate` variable is a bit vague - the name of the variable provides no
   clue that "d" = "death", and the values (e.g. 75) are ambiguous.
    1. [Rename](03-data-management.html#renaming-variables) `drate` to
       `deathrate`.
    2. The rate is actually per 10,000 individuals.
       [Label](03-data-management.html#labeling-variables) `dearthrate` to
...
3. The variable `region` has a value label associated with it ("cenreg"). It has
   some inconsistent choices, namely "NE" and "N Cntrl". Fix this.
    1. [Create a new value label](03-data-management.html#labeling-values) which
...
[later](04-data-manipulation.html#conditional-variable-generation), in Stata,
...
Using [`summarize` and `codebook`](03-data-management.html#summarizing-the-data)
to explore the "census9" data (use the version you saved
[earlier](03-data-management.html#exercise-3)!) and answer the following
questions:
